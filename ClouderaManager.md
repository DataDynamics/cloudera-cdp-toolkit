...
| Server | 구성 관리   | 서비스 설정 생성 및 배포           |
| Server | 모니터링    | 각 노드의 상태 및 리소스 사용 정보 수집  |
| Server | 알림 및 로그 | 이벤트 감지 및 로그 통합           |
| Server | 보안        | Kerberos, TLS, 사용자 권한 관리 |
| Server | 패키지 관리  | Parcel 기반 설치, 업그레이드, 롤백  |
| Server | 인터페이스   | 웹 UI 및 REST API 제공       |
| Agent | 서비스 제어 | CM Server 명령에 따라 서비스 실행/중지/재시작 |
| Agent | 구성 배포  | 서버로부터 설정 파일을 받아 배포             |
| Agent | 상태 보고  | 각 서비스 상태 및 리소스 사용 정보를 주기적으로 보고 |
| Agent | 명령 실행  | 설치, 업그레이드, 진단 명령 등을 실행         |
| Agent | 보안 구성  | Kerberos/TLS 관련 설정을 배포 및 적용    |
| Agent | 패키지 설치 | 서비스 바이너리 또는 parcel 설치 수행       |
| Supervisor | 서비스 실행     | DataNode, YARN NodeManager 등 데몬 실행 |
| Supervisor | 프로세스 모니터링  | 비정상 종료 시 재시작                   |
| Supervisor | 상태 보고      | 메모리, CPU, 상태 정보를 CM Server에 전달   |
| Supervisor | 명령 실행      | 구성 변경, 롤링 재시작 등의 명령 수행        |
| Supervisor | 시스템 안정성 유지 | 프로세스 감시 및 보안 설정 적용          |
